...
{
    public function calculateBalance($date ,$updateIncome = null, $updateExpense = null,$eventName = null)
    {
        ...
        foreach ($balancesData->get() as $balance) {
            ...
            {
                $balance->total_income = $balance->total_income + $updateIncome;
                $balance->save();
            }
        }
        }
        ...
        {            
                foreach ($balancesData->get() as $balance) {
                    if ($date < $balance->date) {
                        $balance->total_expenses = $balance->total_expenses + $updateExpense;
                        $balance->save();
                    }
                }
        }
    ...
    public function calculateBalanceOnDelete($date ,$updateIncomeDifference=null,$updateExpenseDifference=null,$eventName = null)
    {
        ...
        $checkBalancesData = Balance::where('user_id', Auth::user()->id)->get();

        if($checkBalancesData != null){

            foreach ($checkBalancesData as $balance) {
                if ($balance->total_income == 0 && $balance->total_expenses == 0) 
                {  
                    $balance->delete();
